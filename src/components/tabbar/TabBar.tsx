...
const TabBarButton = styled.TouchableOpacity<{ active: boolean }>`
  ...
  background-color: ${(props) =>
    props.active
      ? props.theme.TabBar.active
      : props.theme.TabBar.backgroundColor};
`;

const TabBarText = styled.Text<{ active: boolean }>`
  color: ${(props) =>
    props.active
      ? props.theme.TabBar.activeText
      : props.theme.TabBar.textColor};
`;
...
export const TabBar: React.FunctionComponent<BottomTabBarProps> = (props) => {
  return (
    <TabBarContainer
      ...
      columns={props.state.routes.length}
    >
      {props.state.routes.map((route, index) => {
        ...
        return (
          ...
          >
            <TabBarText active={isFocused}>{label}</TabBarText>
          </TabBarButton>
        );
      })}
    </TabBarContainer>
